...
float exposureLevel = 1.0f; // Data 0
...
float4 PSScaleBuffer(float2 tex :TEXCOORD0): COLOR				
{
   ...
   rgba = main(w2.rrrr, rgba*luminance);
   return rgba;
}
...
technique Exposure
{
   pass P0
   {
      VertexShader = compile vs_3_0 VSRenderTarget();
      PixelShader  = compile ps_3_0 PSExposure();
   }
}
...
technique Tonemap
{
   pass P0
   {
      VertexShader = compile vs_3_0 VSTonemap();
      PixelShader  = compile ps_3_0 PSTonemap();
   }
}
